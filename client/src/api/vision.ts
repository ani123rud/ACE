...
export async function saveReferenceFace(sessionId: string, imageBase64: string) {
  const { data } = await api.post('/api/vision/reference', { sessionId, imageBase64 }, { timeout: 30000 });
  return data as { ok: boolean; refId: string; meta: any };
}

export async function verifyFrame(sessionId: string, imageBase64: string) {
  const { data } = await api.post('/api/vision/verify', { sessionId, imageBase64 }, { timeout: 30000 });
  return data as {
    ok: boolean;
...
export async function createReference(imageBase64: string) {
  const { data } = await api.post('/api/vision/reference', { imageBase64 }, { timeout: 30000 });
...
export async function saveReferenceEmbedding(sessionId: string, embedding: number[], meta?: any) {
  const { data } = await api.post('/api/vision/reference/save', { sessionId, embedding, meta }, { timeout: 30000 });
